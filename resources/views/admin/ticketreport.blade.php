                ...
                <div class="tickets-card">
                    ...
                    <div class="table-header-box">
                        ...
                        <form action="{{ route('admin.ticketreport') }}" method="GET" class="tr-controls" id="filterForm">
                            ...
                            <div style="position: relative; display: inline-block;">
                                <select name="technician" onchange="document.getElementById('filterForm').submit()" style="padding: 0.5rem 2.5rem 0.5rem 1rem; border-radius: 6px; border: 1px solid #e5e7eb; font-size: 0.85rem; font-weight: 600; color: #374151; background: #f9fafb; cursor: pointer; outline: none; appearance: none; transition: all 0.2s;">
                                    @if(isset($technicians) && $technicians->count() > 0)
                                        <option value="">Technicians</option>
                                            @foreach($technicians as $tech)
                                                <option value="tech_{{ $tech->id }}" {{ request('technician') == 'tech_'.$tech->id ? 'selected' : '' }}>{{ $tech->name }}</option>
                                            @endforeach
