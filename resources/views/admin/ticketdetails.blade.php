...
<body>
            ...
            <div class="dashboard-content ticket-details-view">
                <div class="ticket-details-main">
                    <div class="td-left-card">
                        <div class="td-header"><span class="td-breadcrumb">TICKET DATABASE <i
                                    class="fa-solid fa-chevron-right"></i> <span class="td-id">ID:
                                    {{ $ticket->ticket_id }}</span></span></div>

                        <div class="td-attributes">
                            <div class="td-attr-row">
                                <div class="td-attr"><span class="td-lbl">STATUS:</span><span
                                        class="td-pill pill-{{ str_replace(' ', '', $ticket->status) }}">{{ strtoupper(str_replace('_', ' ', $ticket->status)) }}</span>
                                </div>
                            </div>
                            <div class="td-attr-row">
                                <div class="td-attr"><span class="td-lbl">EMPLOYEE</span><span
                                        class="td-pill pill-grey">{{ $ticket->user->name }}</span></div>
                            </div>
                            <div class="td-attr-row">
                                <div class="td-attr"><span class="td-lbl">PRIORITY:</span><span
                                        class="td-pill pill-{{ $ticket->priority }}">{{ strtoupper($ticket->priority) }}</span>
                                </div>
                            </div>
                            <div class="td-attr-row">
                                <div class="td-attr"><span class="td-lbl">TECHNICIAN</span><span
                                        class="td-pill pill-grey">{{ $ticket->technician ? $ticket->technician->name : 'UNASSIGNED' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($ticket->status === 'resolved' || $ticket->status === 'closed')
                            <div class="td-description-sec"
                                style="margin-top: 20px; border: 2px solid #059669; border-radius: 12px; padding: 1.5rem; background: #f0fdf4;">
                                ...
                                <div style="font-size: 0.8rem; color: #6b7280; margin-top: 10px;">
                                    Resolved on
                                    {{ $ticket->resolved_at ? $ticket->resolved_at->format('M d, Y h:i A') : 'N/A' }}
                                </div>
                            </div>
                        @endif
                        ...
                        @if ($ticket->rejection_note)
                            <div class="td-description-sec"
                                style="margin-top: 15px; border: 2px solid #dc2626; border-radius: 12px; padding: 1.5rem; background: #fef2f2;">
                        ...
                        @if ($ticket->attachments->count() > 0)
                            <div class="td-attachment-carousel">
                                <div class="td-carousel-container" onclick="openLightbox(currentIndex)"
                                    style="position: relative;">
                                    <div id="mainAttachmentDisplay"
                                        style="width: 100%; height: 300px; display: flex; align-items: center; justify-content: center; overflow: hidden; border-radius: 8px; background: #f3f4f6;">
                                        @php $firstAttachment = $ticket->attachments->first(); @endphp
                                        @if (str_contains($firstAttachment->mime_type, 'image'))
                                            <img src="{{ Storage::url($firstAttachment->file_path) }}"
                                                alt="Ticket Attachment" id="mainAttachmentImg"
                                                style="width: 100%; height: 100%; object-fit: cover;">
                                        @else
                                            <div class="video-placeholder"
                                                style="display: flex; align-items: center; justify-content: center;">
                                                <i class="fa-solid fa-circle-play"
                                                    style="font-size: 3rem; color: #4b5563;"></i>
                                                <p
                                                    style="margin-left: 10px; margin-bottom: 0; color: #4b5563; font-weight: bold;">
                                                    Click to play video</p>
                                            </div>
                                        @endif
                                    </div>

                                    @if ($ticket->attachments->count() > 1)
                                        <div class="round-btn right-btn" id="nextAttachment"><i
                                                class="fa-solid fa-chevron-right"></i></div>
                                        <div class="round-btn left-btn" id="prevAttachment"><i
                                ...
                                <div class="td-attachment-info">
                                    <div class="att-name" id="attachmentName"><i class="fa-regular fa-file"></i>
                                        {{ $firstAttachment->original_name }}</div>
                                    <div class="att-time">Uploaded
                                        {{ $firstAttachment->created_at->format('M d, h:i A') }}</div>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="td-meta-footer" style="display: flex; flex-direction: column; gap: 1.5rem;">
                            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                                <div class="td-meta-box"><span class="meta-lbl">DATE CREATED</span><span
                                        class="meta-val">{{ $ticket->created_at->format('M d, Y - h:i A') }}</span>
                                </div>
                                <div class="td-meta-box"><span class="meta-lbl">CATEGORY</span><span
                                        class="meta-val">{{ $ticket->category ? $ticket->category->name : 'N/A' }}</span>
                                </div>
                                <div class="td-meta-box"><span class="meta-lbl">INCIDENT TYPE</span><span
                                        class="meta-val">{{ $ticket->custom_type ?? ($ticket->type ? $ticket->type->name : 'N/A') }}</span>
                                </div>
                            </div>
                            <div style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: flex-end;">
                                <div class="td-meta-box" style="flex: 2;"><span class="meta-lbl">SUBJECT</span><span
                                        class="meta-val">{{ $ticket->subject }}</span></div>

                                @if ($ticket->due_at && !in_array($ticket->status, ['resolved', 'closed']))
                                    <div class="td-meta-box"
                                        style="background: #fff7ed; border: 1px solid #ffedd5; flex: 1;">
                                        <span class="meta-lbl" style="color: #9a3412;">DEADLINE COUNTDOWN</span>
                                        <span class="meta-val" id="slaCountdown"
                                            style="color: #c2410c; font-weight: 800; font-family: monospace; font-size: 1.1rem;">--:--:--</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="td-right-panel">
                        <div class="comm-header">
                            ...
                            <div class="comm-meta"><span class="status-dot"></span> <span id="onlineCount">1</span>
                                PARTICIPANTS ONLINE</div>
                        </div>
                        <div id="chatHistory" class="comm-history"
                            style="height: 400px; overflow-y: auto; padding: 1rem;"></div>
                        <div class="comm-input-area">
                            ...
                            <div class="comm-actions">
                                ...
                                <button class="send-msg-btn" id="sendMessageBtn">Send <i
    ...
    <div id="lightbox" class="lightbox-modal">
        <div class="lightbox-content">
            <div class="lightbox-header"><span id="lightboxCounter">1/1</span><span class="lightbox-close"
                    onclick="closeLightbox()">&times;</span></div>
            <div class="lightbox-main" id="lightboxMain"></div>
            @if ($ticket->attachments->count() > 1)
                <button class="lightbox-nav lightbox-prev" onclick="changeLightbox(-1)"><i
                        class="fa-solid fa-chevron-left"></i></button>
                <button class="lightbox-nav lightbox-next" onclick="changeLightbox(1)"><i
    ...
    <script>
        ...
        function renderLightboxContent() {
            ...
            else lightboxMain.innerHTML =
                `<video controls autoplay style="max-height: 70vh; border-radius: 12px;"><source src="${att.url}" type="${att.mime}">Your browser does not support the video tag.</video>`;
        }
        ...
        function updateMainAttachment(index) {
            ...
            if (att.mime.includes('image')) {
                display.innerHTML =
                    `<img src="${att.url}" alt="Ticket Attachment" id="mainAttachmentImg" style="width: 100%; height: 100%; object-fit: cover;">`;
            } else {
                display.innerHTML = `
        ...
        if (nextBtn) nextBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            updateMainAttachment(currentIndex + 1);
        });
        ...
        function fetchMessages() {
            fetch(`/tickets/${ticketId}/messages?t=` + new Date().getTime(), {
                ...
                .then(res => {
                    // If server returned non-2xx, throw so .catch() handles it
        ...
        // Append single message to the chat history
...
// prevents sending empty messages (with no content and no files)
            if (!content && files.length === 0) {
                return;
            }
// creates new form data object
            const fd = new FormData();
// appends the content of the message
...
// send form data
        ...
        // REALTIME SOCKET LISTENER ( instant updates when Reverb is running)
        try {
            if (window.Echo) {
